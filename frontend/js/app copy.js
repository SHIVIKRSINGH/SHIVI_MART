...
class App {
  ...
  async init() {
    ...
    if (token) {
      ...
      if (addresses && addresses.addresses.length > 0) {
        // User has address in DB → Main screen
        await this.loadMainScreen();
      } else {
        // User needs address → Address screen
        await this.showAddressScreen();
      }
    } else {
  ...
  async handleOTPVerify(e) {
    ...
    // ✅ Check DATABASE for addresses
    const addresses = await API.societies.getMyAddresses();

    if (addresses && addresses.addresses.length > 0) {
      // Has address → Main screen
      await this.loadMainScreen();
    } else {
      // No address → Address screen
      await this.showAddressScreen();
    }
  }
  ...
  async handleAddressSubmit(e) {
    ...
    // Address saved in DB → Go to main screen
    await this.loadMainScreen();
}
